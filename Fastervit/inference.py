...
torch.set_num_threads(1) # Try 1, 2, or 4. Usually 1 or 2 is best for per-sample latency.
# ==================== Profiling Imports ====================
try:
...
def fuse_model(model):
    ...
    # PART 1: Fuse Convolutions (This was already in your code)
    ...
    # PART 2: Fuse Linear Layers (THIS WAS MISSING)
    # We must manually iterate through the 'head' Sequential block
    head = model.head
    i = 0
    while i < len(head) - 1:
        # Check if we have Linear followed by BatchNorm1d
        if isinstance(head[i], nn.Linear) and isinstance(head[i+1], nn.BatchNorm1d):
            ...
            # Update the Linear layer with fused values
            head[i].weight.data = w_fused
            head[i].bias.data = b_fused
            
            # Delete the BatchNorm layer (replace with Identity)
            head[i+1] = nn.Identity()
            
            i += 2 # Skip the next layer since we just fused it
        else:
            i += 1
# ==================== MAIN ====================
...
if __name__ == "__main__":
    ...
    # [FIX] Fixed Indentation here
    try:
        fuse_model(model)
    except Exception as e:
        print(f"Fusion skipped or failed: {e}")
        pass
