...
class StreamlinedHAT(nn.Module):
    ...
    def forward(self, x, ct):
        ...
        attn = (attn * self.pos_scale.view(1, -1, 1, 1)).softmax(dim=-1)
...
def extract_ctus_vectorized(luma, padded_height, padded_width):
    """
    OPTIMIZED: Vectorized CTU extraction using NumPy reshape/transpose
    ~10-50x faster than nested loops
    """
    # Pad frame to multiples of 64 using edge replication
    h, w = luma.shape
    if h < padded_height or w < padded_width:
        luma = np.pad(luma, ((0, padded_height - h), (0, padded_width - w)), mode='edge')
    
    # Vectorized extraction: reshape and transpose to extract all CTUs at once
    num_rows = padded_height // IMAGE_SIZE
    num_cols = padded_width // IMAGE_SIZE
    
    # Reshape: [H, W] -> [num_rows, 64, num_cols, 64] -> [num_rows, num_cols, 64, 64]
...
def process_frame_batch(model, ctus, qp_value, batch_size=256):
    ...
    - Smaller batch size (256) for better L3 cache utilization
    - Direct tensor operations (no intermediate numpy conversions)
    - Immediate result writing
    """
    num_ctus = ctus.shape[0]
    results = []
    
    # Pre-normalize all CTUs at once (faster than per-batch)
    ctus_normalized = torch.from_numpy(ctus).float().div_(255.0).unsqueeze(1)
    ctus_normalized = ctus_normalized.to(memory_format=torch.channels_last)
    
    # Create QP tensor once
    ...
    try:
        with open(yuv_file, 'rb') as f_in, open(save_file, 'wb') as f_out:
            while True:
                ...
                # Extract CTUs (VECTORIZED - FAST!)
                ctus = extract_ctus_vectorized(luma, padded_height, padded_width)
                
                # Run inference
                predictions = process_frame_batch(model, ctus, qp_value, batch_size=256)
...
def load_model(checkpoint_path):
    """Load fused model with proper error handling"""
...
def main():
    print("=" * 70)
    print("FasterViT HEVC CU Partition Prediction (OPTIMIZED)")
    print("=" * 70)
