...
VALID_FILE_PATH = "/home/krishna/my_project/Data/1080p_dataset/AI_Valid_9600.dat_shuffled"

# Checkpoint Filenames
CHECKPOINT_PATH = 'best_fastervit_hevc_balanced.pth'
HISTORY_CHECKPOINT_PATH = 'history_fastervit.pth'
...
class BalancedFasterViT_HEVC(nn.Module):
    ...
    def forward(self, x, qp):
        ...
        # Step 1: HAT1 - CTs collect local info from windows
        x, ct = self.hat1(x, ct)

        # Step 2: CT Interaction - CTs share info with each other
        ct = self.ct_interaction(ct, B)

        # Step 3: HAT2 - CTs distribute GLOBAL info back to windows
        x, _ = self.hat2(x, ct)
        # ─── END CHANGE ───
...
if os.path.exists(HISTORY_CHECKPOINT_PATH):
    ...
    if start_epoch > 0:
        ...
        # B. Truncate Validation Metrics (recorded sparsely)
