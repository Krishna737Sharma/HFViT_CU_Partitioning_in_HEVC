...
# Using the modified QP-Aware MobileFormer Model
# ----------------------------------------------------------------------------------
...
# Import the modified MobileFormer model
# Make sure your files are in a directory structure that allows this.
# e.g., all in the same folder or in a package
from mobile_former_ctu import mobile_former_294m # Using 294M as default
from registry import is_model, model_entrypoint # Need these if not in a package
...
class StreamingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        with open(self.file_path, 'rb') as file_reader:
            ...
            # Normalize
            ctu_tensor /= 255.0
            qp_tensor /= 51.0 # *** CRITICAL: Normalize QP for the model ***
...
def calculate_accuracy_repo(y_flat_64, y_conv_flat_64,
    ...
    count = 1.0
    if torch.sum(y_flat_valid_32) > 0:
        count += 1.0
    if torch.sum(y_flat_valid_16) > 0:
        count += 1.0
...
def main():
    ...
    BATCH_SIZE = 64
    MODEL_VARIANT = 'mobile_former_294m'
    
    # --- Initialize wandb ---
    wandb.login(key="5c560f0045b5a49dcf8caa862e58469329427192") # Use your key
    ...
    wandb.init(
        project="MobileFormer-HEVC-Partition", 
        config=config
    )
    ...
    # --- DataLoaders ---
    # Using smaller subsets for demonstration
    ...
    # We use the function from mobile_former_ctu.py
    # The __init__ defaults are already set for 64x64, 1-ch, 21-classes
    model = mobile_former_294m(pretrained=False,in_chans=NUM_CHANNELS,num_classes=NUM_CLASSES).to(device)
    
    wandb.watch(model)
    print(f"Model Parameters: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")

    optimizer = optim.AdamW(model.parameters(), lr=config['learning_rate'], weight_decay=0.025)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=config['epochs'])
    
    criterion = nn.BCELoss()
    
    # --- Checkpoint and Patience Variables ---
    best_val_acc = 0.0 # *** Use accuracy for saving ***
    start_epoch = 0
    patience = 50 
    patience_counter = 0
    checkpoint_path = f'best_mobileformer_ctu_model.pth'
    ...
    if os.path.exists(checkpoint_path):
        ...
        try:
            ...
            best_val_acc = checkpoint.get('best_val_acc', 0.0) # Load best acc
    ...
    for epoch in range(start_epoch, config['epochs']):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # *** CRITICAL: Pass both inputs and qp_tensor to the model ***
        ...
        with torch.no_grad():
            for val_batch in validation_loader:
                ...
                avg_acc, _, _, _ = calculate_accuracy_repo(
                    y_flat_64_dev, outputs[:, 0:1],       
                    y_flat_32_dev, outputs[:, 1:5], y_flat_valid_32_dev, 
                    y_flat_16_dev, outputs[:, 5:21], y_flat_valid_16_dev
                )
                if not torch.isnan(avg_acc): 
        ...
        if avg_val_acc > best_val_acc: # Save based on best accuracy
