""" Dna blocks used for Mobile-Former (MODIFIED FOR CTU TASK)
...
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# +++ START: QP-AWARE MODULES FOR CTU TASK ++++++++++++++++++++++++
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

class FFN_with_QP(nn.Module):
    """
    Feed-Forward Network modified to accept a QP value.
    Operates on TBC format (Token, Batch, Channel)
    """
    def __init__(self, in_features, hidden_features, out_features):
        super().__init__()
        # First linear layer takes in_features + 1 (for QP)
    ...
    def forward(self, x, qp):
        ...
        # Expand qp: [bs] -> [1, bs, 1] -> [T, bs, 1]
        qp_expanded = qp.unsqueeze(0).unsqueeze(2).expand(T, bs, 1)
...
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# +++ END: QP-AWARE MODULES FOR CTU TASK ++++++++++++++++++++++++++
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
class GlobalBlock(nn.Module):
    def __init__(
        ...
        if self.use_ffn:
            # print('use ffn')
            # *** MODIFIED FOR QP-AWARE FFN ***
            self.ffn = FFN_with_QP(
                in_features=token_dim,
                hidden_features=token_dim * self.ffn_exp,
    ...
    def forward(self, x, qp): # *** MODIFIED: Added 'qp' ***
        ...
        if self.use_ffn:
            # *** MODIFIED: Pass 'qp' to FFN ***
            t_ffn = self.ffn(tokens, qp)
            tokens = tokens + t_ffn
...
class DnaBlock3(nn.Module):
    ...
    def forward(self, x_in): # *** MODIFIED: Accept a single tuple ***
        features, tokens, qp = x_in # *** MODIFIED: Unpack tuple ***
        ...
        if self.use_conv_alone:
            ...
            # *** MODIFIED: Pass 'qp' to global_block ***
            tokens = self.global_block(tokens, qp)
